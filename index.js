...
const channelId = "776624477752524850";
const rulesChannel = "776628467830882334";
client.on("guildMemberAdd", (member) => {
    console.log(member);

    const descrição = `Seja bem-vindo(a) <@${
      member.id
    }> ao servidor do jaré! Estamos atualmente com ${guild.approximateMemberCount
} Lembre-se de olhar o ${member.guild.channels.cache
      .get(rulesChannel)
      .toString()}`;
...
client.on("message", async(message) => {
    ...
    function Queue(serverQueue){
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setColor("#7DEB5E")
        .setDescription(`${qMsg} Requerido por: ${message.author}`)
